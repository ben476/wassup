export default function ListItem({ email = {}, ...props }: { email?: EmailType, [key: string]: any }) {
    return <a
        ...
        key={email.id}
        {...props}
        className={"focus-watch font-medium flex flex-row px-8 py-4 gap-4 rounded-none border-b dark:border-none border-gray-200 focus:bg-white focus:dark:bg-gray-800 focus:shadow focus:outline-none transition " + (email.opened ? "font-medium " : "font-normal ") + (props.className || "")}
    >
